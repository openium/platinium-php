<?php

require_once(__DIR__ . '/../api/PPSNotifier.php');

$apiServerId = 'b9ca566f-0570-ed44-31ab-106ceac0bf3c';
$apiServerKey = '';
$apiServerTokenDev = '43b4f716f8ef4f0b5ddda0d09d35c5d72164daa6';
$apiServerTokenProd = 'a57c2c00eec8311b3969905fc0fe602e2253547f';

$groups = array('iosdroid1');
$langs = array('fr', 'en');
$version = '1.0';
$operator = '=';

$langsStr = ($langs == null) ? 'all languages' : implode(',', $langs) . ' version ' . $operator . ' ' . $version;
$notificationMessage = date(DATE_RFC822) . ' - Hello ' . implode(',', $groups) . ' (' . $langsStr . ')';
$notification = new PPSNotification($notificationMessage, 1);

$notifier = new PPSNotifier($apiServerId, $apiServerKey, $apiServerTokenDev, $apiServerTokenProd);
$notifier->setServer('http://platinium-dev.openium.fr');
$notifier->setAppVersion($version);
$notifier->setVersionOperator($operator);
$response = $notifier->notify($notification, PPSNotifier::ENV_DEV, $groups, $langs);

var_dump($response);

?>
